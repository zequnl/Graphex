...
import numpy as np
import math
from model.common_layer import EncoderLayer, DecoderLayer, MultiHeadAttention, Conv, PositionwiseFeedForward, LayerNorm , _gen_bias_mask ,_gen_timing_signal, share_embedding, LabelSmoothing, NoamOpt, _get_attn_subsequent_mask,  get_input_from_batch, get_output_from_batch, get_text_from_batch
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, encoder_output, mask):
        ...
        if(self.universal):
            ...
            # Add timing signal
            #print("decode", inputs.size())
            #print(
            #print("decode", self.timing_signal[:, :inputs.shape[1], :].size())
            x += self.timing_signal[:, :inputs.shape[1], :].type_as(inputs.data)
...
class Generator(nn.Module):
    ...
    def forward(self, x, attn_dist=None, enc_batch_extend_vocab=None, extra_zeros=None, temp=1, beam_search=False):
        ...
        if config.pointer_gen and extra_zeros is not None:
            ...
            #print(enc_batch_extend_vocab_.size())
            #print(vocab_dist_.size())
            #print(attn_dist_.size())
